...
const transfer_aggregate_type_model_1 = require("lqd-wallet-client/models/transactions/transfer-aggregate-type.model");
const fs = require("fs");
...
function signMessageRSV(data, privKey) {
    ...
    const messageLqdWrapped = web3_utils_1.default.soliditySha3('\x19Liquidity.Network Authorization:\n32', data);
    const message = ethereumjs_util_1.toBuffer(messageLqdWrapped);
...
function transferDataToTransfer(isIncomingPending) {
    return (transferData) => {
        const transfer = new lqd_wallet_client_1.Transfer(transferData.wallet.token, transferData.wallet.address, transferData.round, new bignumber_js_1.default(transferData.amount), null, null, transferData.recipient.token, transferData.recipient.address, transferData.recipient_trail_identifier, transferData.wallet.address, new bignumber_js_1.default(transferData.nonce), transferData.id);
        if (isIncomingPending) {
            transfer.authorizations.set(transfer_aggregate_type_model_1.TransferAggregateType.DEBIT, transferData.sender_aggregate);
        }
        return transfer;
    };
}
...
const init = () => __awaiter(this, void 0, void 0, function* () {
    const contractOwner = yield lqdClient.contractApiService.getHubOwnerAddress(config.HUB_CONTRACT_ADDRESS);
    clientWallet = new lqd_wallet_client_1.Wallet(address, parseInt(config.ETHEREUM_NETWORK_ID), contractOwner, config.HUB_CONTRACT_ADDRESS, config.TOKEN_CONTRACT_ADDRESS, config.HUB_PROVIDER_URL);
    let walletState;
                ...
                if (wallet.latestRound.aggregate.pendingTransfer) {
                    ...
                    for (let i = wallet.registrationRound; i <= wallet.latestRound.roundNumber; i++) {
                        ...
                        for (let tx of round.aggregate.transfers) {
                            if (!util_1.isSameHexValue(tx.recipient, address) || !tx.confirmations.get(transfer_aggregate_type_model_1.TransferAggregateType.CREDIT)) {
                                continue;
                            }
                            paymentsMap.set(tx.txId, {
                                ...
                                sender: tx.sender,
                                status: transaction_status_1.default.fromBoolean(typeof tx.confirmations.get(transfer_aggregate_type_model_1.TransferAggregateType.CREDIT) !== 'undefined')
                            });
                        }
                    }
